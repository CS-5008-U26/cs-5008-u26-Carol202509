...
void bstInorderTraversal (bstNode* root, int a[], int* index) {
    if (root == NULL) {
        return;
    }
    bstInorderTraversal(root->left, a, index);
    a[*index] = root->value;
    (*index)++;
    bstInorderTraversal(root->right, a, index);
}
...
void sortarray (int a[], int n) {
    ...
    // refill the array with the sorted values from the bst
    int index = 0;
    bstInorderTraversal(root, a, &index);
    freeBST(root);
}
...
int main () {
    ...
    int keepgoing = 1;
    while (keepgoing) {
        printf ("How many elements(random data)? ");
