/*
* print the percentage of chocolate candies that also have caramel,
* print the names of the chocolate candies with caramel in uppercase
* and the names of the chocolate candies without caramel in lowercase.
*
*/
...
void printUpper(char *str) {
    for (int i = 0; str[i] != '\0'; i++) {
        putchar(toupper(str[i])); // Print each character in uppercase
    }
...
void printLower(char *str) {
    for (int i = 0; str[i] != '\0'; i++) {
        putchar(tolower(str[i])); // Print each character in lowercase
    }
    putchar('\n'); // Print a newline after the string
}
...
int main() {
    ...
    for (int i =0; i < countLines; i++) {
        if (myCandyList[i]->containsChocolate) {
            chocolateCount++; // Increment the chocolate candy count
            if (myCandyList[i]->containsCaramel) {
                chocolateCaramelCount++; // Increment the chocolate and caramel candy count
                printUpper(myCandyList[i]->name); // Print the name of the chocolate and caramel candy in uppercase
            }
            else {
                printLower(myCandyList[i]->name); // Print the name of the chocolate candy in lowercase
            }
        }
    }

    percent = (double)chocolateCaramelCount / chocolateCount * 100;
    printf("\n%.1f%% of chocolate candies also have caramel.\n", percent);
    ...
    free(myCandyList); // Free the array of pointers to candy structures
