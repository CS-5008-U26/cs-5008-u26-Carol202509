/*
* print summary information from the file "candy-data.csv".
*
*/
...
int main() {
    ...
    printf("\n==== Summary Information ====\n");
    char *attributeNames[] = {"Chocolate", "Fruity", "Caramel", "Nuts", "Nougat", "Rice Wafers/Cookies", "Hard", "Bar", "Pluribus"};
    double sumSugar[9] = {0.0};
    double sumPrice[9] = {0.0};
    double sumWin[9] = {0.0};
    int attributeCount[9] = {0};

    for (int i = 0; i < countLines; i++) {
        int attributes[9] = {
            myCandyList[i]->containsChocolate,
            myCandyList[i]->isFruity,
            myCandyList[i]->containsCaramel,
            myCandyList[i]->containsNuts,
            myCandyList[i]->containsNougat,
            myCandyList[i]->containsRiceWafersCookies,
            myCandyList[i]->isHard,
            myCandyList[i]->isBar,
            myCandyList[i]->isPluribus
        };
        for (int j = 0; j < 9; j++) {
            if (attributes[j]) {
                attributeCount[j]++;
                sumSugar[j] += myCandyList[i]->sugarPercent;
                sumPrice[j] += myCandyList[i]->pricePercent;
                sumWin[j] += myCandyList[i]->winPercent;
            }
        }
    }
    for (int j = 0; j < 9; j++) {
        if (attributeCount[j] > 0) {
            printf("%s: Count = %d, Average Sugar = %.2f%%, Average Price = %.2f%%, Average Win = %.2f%%\n",
                attributeNames[j], attributeCount[j], sumSugar[j] / attributeCount[j],
                sumPrice[j] / attributeCount[j], sumWin[j] / attributeCount[j]);
        }
    }
    ...
    for (int i = 0; i < countLines; i++) {
        totalSugar += myCandyList[i]->sugarPercent;
        totalPrice += myCandyList[i]->pricePercent;
    }
    avgSugar = totalSugar / countLines;
    avgPrice = totalPrice / countLines;

    // print the above average sugar
    double sumS = 0.0;
    double sumP = 0.0;
    double sumW = 0.0;
    int countAboveAvg = 0;
    ...
    for (int i = 0; i < countLines; i++) {
        if (myCandyList[i]->sugarPercent > avgSugar) {
             countAboveAvg++;
             sumS += myCandyList[i]->sugarPercent;
             sumP += myCandyList[i]->pricePercent;
             sumW += myCandyList[i]->winPercent;
        }
    }
    printf("Above average sugar: Count = %d, Average Sugar = %.2f%%, Average Price = %.2f%%, Average Win = %.2f%%\n",
        countAboveAvg, sumS / countAboveAvg, sumP / countAboveAvg, sumW / countAboveAvg);

    // print the above average price
    sumS = 0.0;
    sumP = 0.0;
    sumW = 0.0;
    countAboveAvg = 0;
    ...
    for (int i = 0; i < countLines; i++) {
        if (myCandyList[i]->pricePercent > avgPrice) {
             countAboveAvg++;
             sumS += myCandyList[i]->sugarPercent;
             sumP += myCandyList[i]->pricePercent;
             sumW += myCandyList[i]->winPercent;
    ...
    printf("Above average price: Count = %d, Average Price = %.2f%%, Average Sugar = %.2f%%, Average Win = %.2f%%\n",
        countAboveAvg, sumP / countAboveAvg, sumS / countAboveAvg, sumW / countAboveAvg);

    // Free allocated memory
    for (int i = 0; i < countLines; i++) {
        free(myCandyList[i]->name); // Free the name string
        free(myCandyList[i]); // Free the candy structure
    }
    free(myCandyList);
